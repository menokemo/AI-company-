...
- `pipeline.py`: bug حرج كان يجعل كل الـ 6 agents يستخدمون "claude" دائماً، متجاهلين اختيار لوحة التحكم تماماً
- `crew.py`/`server.py`: نفس النمط لموديل OpenHands ("المبرمج")
- إزالة كل القيم المكتوبة يدويًا (IP شخصي، روابط) واستبدالها بقراءة ديناميكية من الإعدادات
- حذف كود ميت غير مستخدم: `code-writer/`, `llm-gateway/`
- إصلاح "Sync from Infisical" بحيث تظهر المفاتيح الجديدة فوراً بدون الحاجة لإعادة تشغيل يدوية
- أتمتة إعداد LLM الخاص بـ OpenHands (لم يكن يحدث تلقائياً بعد كل تثبيت/مسح)
- إعادة محاولة تلقائية لمشكلة race condition معروفة في OpenHands نفسه (sandbox startup)
- إعادة محاولة تلقائية مع انتظار لـ rate-limit الموديلات المجانية في OpenRouter

### جديد
- **👥 سجل أعمال الموظفين**: تبويب جديد في لوحة التحكم يعرض كل تشغيلة سابقة — ماذا أنتج كل agent (محلل النصوص، الباحث، المصمم، المخطط، حلّال المشاكل، المراجع) ونتيجة OpenHands، محفوظ على القرص (ينجو من إعادة تشغيل الـ containers)
- `tools-api`: endpoint جديد `/run-history`
