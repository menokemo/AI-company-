...
## جلسة 2026-06-20 — التصاميم بتطلع متشابهة وغير مرتبطة بنوع المشروع الحقيقي

المستخدم لاحظ: التصاميم تتكرر تقريباً نفس الشكل بين مشاريع مختلفة، وما لها علاقة فعلية بنوع النشاط (مثال: محل نظارات يطلع بتصميم عام بدون أي منتجات نظارات حقيقية).

### السبب
1. الـ prompt كان يذكر `DESCRIPTION` كحقل سياق فقط، بدون ربط صريح وقوي يفرض أن كل محتوى يُولَّد (منتجات، أسعار، فئات) يعكس هذا النشاط بالتحديد
2. توثيق أداة `generate_mockups` كان يطلب من المدير "وصف مشروع مختصر" — صياغة عامة شجّعت على تمرير وصف سطحي لا يحمل سياق العمل الحقيقي الذي تمت مناقشته مع العميل

### الإصلاح
- تقوية الـ prompt في `crew.py`: تعليمات صريحة وحازمة (مع أمثلة) تربط كل محتوى بـ `DESCRIPTION`، مع اختبار ذاتي: "لو محتوى مشروعين مختلفين ممكن يتبادل بدون ما أي شخص يلاحظ، يبقى فشلت"
- تحسين توثيق `generate_mockups` ليطلب وصفاً تفصيليًا حقيقيًا (نوع النشاط، الجمهور، ما يميّزه) بدل سطر واحد عام

---
---

## 🎯🎯 التصحيح النهائي للاكتشاف أعلاه: السبب الحقيقي 100% (الفرضية عن email كانت غير كافية)

الفرضية السابقة (مشكلة `/user/emails`) لم تكن خاطئة بالكامل، لكنها لم تكن السبب الحقيقي وراء توقف كل المحاولات. بعد تطبيق فيكس `git_user_name/email` والمحاولة، **استمر الفشل بالضبط نفس الطريقة**.

### السبب الحقيقي 100% (مؤكَّد بـ traceback كامل من ai-openhands)
```
File "auth_user_context.py", line 98, in get_provider_handler
    assert provider_tokens is not None
AssertionError
```
`provider_tokens` فاضية تمامًا داخل OpenHands. والسبب: خطوة "ربط GitHub" في `infisical-sync.sh` كانت بداخل شرط:
```bash
if [ -n "$GITHUB_TOKEN" ] && [ -n "$GIT_USERNAME" ]; then ...
```
لكن **`GIT_USERNAME` غير محفوظ في `.env` على الإطلاق** (فقط `GITHUB_TOKEN` موجود) — فالشرط كان **يفشل دائماً منذ أول يوم**، وخطوة الربط بالكامل **لم تُنفَّذ ولا مرة واحدة** عبر كل هذه الجلسة (ولا أي جلسة سابقة على الأرجح). نتيجة ذلك: `provider_tokens` فاضية دومًا في OpenHands، فيفشل أي طلب فيه `selected_repository` بـ `AssertionError` داخلي — **بصرف النظر التام عن الموديل المُستخدَم** (هذا ما يفسّر فشل Claude، GPT-4o، وكل موديلات OpenRouter بنفس الطريقة بالضبط).

### الإصلاح النهائي
`infisical-sync.sh`: إزالة الاعتماد على `GIT_USERNAME` كلياً — جلب اسم المستخدم الحقيقي ديناميكياً من GitHub API مباشرة (`GET https://api.github.com/user`) قبل استدعاء `/api/v1/secrets/git-providers`.

### التأكيد
بعد الإصلاح: تم اختبار مشروع حقيقي → `status: READY`، `app_conversation_id` صحيح، **وكومت حقيقي فعلي**: `feat: complete task` (لا "Initial commit" فقط).

### الدرس الأهم من الجلسة كاملة
خطأ بسيط في شرط `if` (متغيّر بيئة غير موجود) ظل مخفياً عبر عشرات الاختبارات لأن النتيجة الظاهرة (status: ERROR) كانت تُفسَّر دائماً كمشكلة في "الموديل" — المتغيّر الذي كان يتغيّر باستمرار في كل تجربة. الخطأ الحقيقي كان في شيء *ثابت لم يتغيّر أبداً طوال كل المحاولات*. عند تكرار نفس الفشل بدقة مع متغيّرات مختلفة تماماً، السؤال الصحيح هو: "ما الذي بقي ثابتاً في كل هذه المحاولات؟" لا "أي موديل المرة الجاية؟"

---
---

## 🎯 الاكتشاف النهائي والحقيقي: لماذا فشلت كل محاولات OpenHands في هذه الجلسة (بصرف النظر عن الموديل)

بعد ساعات من التشخيص (تجربة ٦+ موديلات مختلفة عبر مزوّدين مختلفين، تحديث نسخ OpenHands، اختبارات تحكّم دقيقة) — تبيّن أن **كل النظريات السابقة عن "الموديل" أو "OpenRouter" كانت خاطئة بالكامل**. السبب الحقيقي لا علاقة له بأي موديل إطلاقًا.

### السبب الجذري الحقيقي
عند إنشاء محادثة OpenHands مرتبطة بـ repository (أي محادثة فيها `selected_repository` + `git_provider`)، يحاول OpenHands استدعاء GitHub API لجلب بريد المستخدم (`/user/emails`) لتحديد committer email لعمليات Git. هذا الـ endpoint **غير مدعوم على الإطلاق مع Fine-grained Personal Access Tokens** (النوع المستخدم في هذا المشروع) — يرجع 404 دائمًا. هذا الفشل لا يُعالَج بسلاسة؛ بل يُسقط *كل* عملية إنشاء المحادثة فورًا (`status: ERROR`, `app_conversation_id: None`) — **بصرف النظر تمامًا عن الموديل المُستخدَم**.

### كيف تم التأكد بشكل قاطع
1. فُحص اللوج الخام للـ sandbox container — توقّف بعد أول bash command ناجح (`mkdir`) بدون أي خطأ ظاهر
2. فُحص `start-tasks/search` عبر API — أظهر `status: ERROR` بتوقيت مطابق بالملي ثانية لتوقّف اللوج
3. **التست الحاسم:** محادثة بدون أي repository مرتبط (نفس كل شيء آخر) → نجحت ووصلت لـ `status: READY` بسلاسة. الفرق الوحيد: وجود repository أم لا
4. هذا يفسّر *كل* حالات الفشل في هذه الجلسة: Claude مباشر ✗، GPT-4o مباشر ✗، 4 موديلات OpenRouter مختلفة ✗ — كل واحدة فشلت بنفس الطريقة بالضبط لأن *كلها* كانت مرتبطة بـ repository

### الإصلاح
`infisical-sync.sh`: خطوة جديدة تلقائية تجلب اسم المستخدم الحقيقي من GitHub API مباشرة (`GET /user`، يعمل بدون مشاكل مع fine-grained PAT)، وتضبط `git_user_name`/`git_user_email` في إعدادات OpenHands — فلا يحتاج بعد ذلك لاستدعاء `/user/emails` الفاشل أصلاً.

### الدرس الأهم من هذه الجلسة
عند ظهور نفس الخطأ مع *كل* المتغيّرات المُجرَّبة (هنا: كل الموديلات)، المتغيّر الحقيقي غالبًا ليس أيًا منها — بل شيء *مشترك* بين كل التجارب لم يتغيّر (هنا: وجود repository). اختبار التحكّم الحقيقي (إزالة المتغيّر المشترك المشتبه به وليس فقط تبديل المتغيّر الظاهري) هو ما كشف الحقيقة بعد ساعات من المطاردة في الاتجاه الخاطئ.

---
---

## تحديث (نفس الجلسة) — تأكيد قاطع: ليست مشكلة نسخة قديمة

بعد الاكتشاف أعلاه، تم التحقق من إمكانية أن السبب نسخة قديمة من OpenHands (المستخدم أشار لاختبار سابق ناجح مع DeepSeek v3 عبر OpenRouter في تثبيت قديم تم مسحه):
- تم سحب `ghcr.io/openhands/openhands:latest` فعليًا من جديد — تأكد أنه **نفس الـ digest الحالي بالضبط** (لا يوجد إصدار أحدث متاح)
- تم تحديث `agent-server` من `1.25.0-python` إلى `1.27.0-python` (الأحدث المتاح)
- أُعيد تكرار نفس تجربة التحكّم (Claude عبر OpenRouter) — **نفس التجمّد بالضبط** حصل مرة أخرى

**الخلاصة النهائية:** هذا قيد حالي وحقيقي في أحدث إصدار من OpenHands SDK مع التوجيه عبر OpenRouter (بصرف النظر عن الموديل المستخدم) — وليس نتيجة نسخة قديمة من جانبنا. ربما يكون قد عمل بنجاح في تثبيت سابق إذا كانت تلك النسخة من OpenHands أقدم (قبل تغيير سلوكي في SDK لاحق)، لكن هذا غير قابل للتحقق الآن لأن ذلك التثبيت تم مسحه.

---
---

## جلسة 2026-06-19 — اكتشاف قيد تقني حقيقي: OpenHands لا يعمل بشكل موثوق عبر OpenRouter

بعد تجربة 4 موديلات مختلفة عبر OpenRouter لـ "المبرمج" (`coder`) — `qwen3-coder-next`, `qwen3-coder`, `moonshotai/kimi-k2`, وحتى **`anthropic/claude-sonnet-4.5` نفسه عبر OpenRouter** — كل واحدة منهم تجمّدت بالضبط في نفس النقطة (بعد تنفيذ أول bash command، صفر نشاط بعد كده لأكثر من 5 دقائق، نفس عدد سطور اللوج بالضبط: 34).

### الاستنتاج المؤكَّد بدليل قاطع
المشكلة **ليست في أي موديل بعينه** — حتى Claude نفسه (المُثبَت نجاحه 100% عبر Anthropic API المباشر) تجمّد بنفس الطريقة بالضبط لما استُخدِم عبر OpenRouter. هذا يدل على وجود **عدم توافق حقيقي بين OpenHands SDK (الإصدار الحالي 1.25.0) وصيغة ردود OpenRouter** تحديداً (مختلفة بشكل ما عن الردود المباشرة من Anthropic/OpenAI، رغم أن LiteLLM نفسه ينقل الطلب بنجاح عند الاختبار المباشر).

### القرار
- **"المبرمج" (coder)**: يجب أن يكون دومًا `anthropic/claude-sonnet-4-5` أو `openai/gpt-4o` (مباشر) — **لا** عبر OpenRouter أبداً، لأن هذا قيد تقني في OpenHands نفسه لا علاقة له بكودنا
- **"مدير المشروع" (manager) والـ 6 agents الأخرى**: غير متأثرين بهذا القيد إطلاقًا — لا يستخدمون OpenHands مباشرة (الـ manager فقط ينادي الأداة، والـ agents تتصل بمزوّداتها مباشرة)، فأي موديل OpenRouter يبقى متاحًا لهم بأمان كامل

### الدرس المستفاد
ليس كل قيد هو bug في كودنا يمكن إصلاحه — بعض القيود حقيقية في الأدوات الخارجية نفسها (هنا: عدم توافق OpenHands SDK مع تنسيق ردود OpenRouter). التشخيص الصحيح هنا تطلّب اختبار تحكّم نظيف (نفس الموديل، طريقين مختلفين) لإثبات أن المتغيّر الحقيقي هو "الطريق" لا "الموديل".

---
---

## جلسة 2026-06-19 — التصاميم (mockups) بتطلع صفحة واحدة بسيطة بدل ٤+ صفحات

المستخدم لاحظ إن كل تصميم من الـ 3 المُولَّدة كان صفحة واحدة بسيطة جدًا، رغم إن الـ prompt كان أصلاً يطلب "4-5 screens".

### السبب الجذري
`max_tokens=4096` في استدعاء توليد التصميم — قليل جدًا لتوليد HTML حقيقي لـ٤-٥ شاشات (تنقّل JS، محتوى واقعي، Tailwind). الموديل كان يبدأ صحيح بس يُقطَع وسط الشغل (token limit)، فيرجع غالباً الشاشة الأولى بس مكتملة.

### الإصلاح
- زيادة `max_tokens` من 4096 إلى 8000 لكل من مسار `litellm.completion()` ومسار `requests` المباشر
- إعادة صياغة الـ prompt بشكل أكثر حزماً: تأكيد صريح إن أقل من 4 شاشات = رد فاشل، وطلب "تحقّق ذاتي" من الموديل قبل الإرجاع (verify 4+ screens exist before returning)

---
---

## جلسة 2026-06-18 (تابع) — مدير المشروع بيكرر نفس الرد ومتجاهل كلام المستخدم

المستخدم اختار موديل OpenRouter محدّد (`deepseek/deepseek-v3.2`) لمدير المشروع، لكن الردود كانت غريبة (تكرار نفس الترحيب رغم إجابة المستخدم بالتفصيل).

### السبب الجذري
`PROVIDER_TO_ALIAS` كانت تحوّل **أي** موديل OpenRouter (بغض النظر عن أي موديل تحديدًا) إلى alias واحد ثابت اسمه `openrouter-auto`، المعرّف في `litellm-config.yaml` كـ `openrouter/auto` — وهو خاصية OpenRouter **الخاصة بيهم** لاختيار موديل تلقائيًا في كل طلب. النتيجة: اختيار المستخدم الفعلي (`deepseek-v3.2`) كان يُتجاهل تمامًا، وكل رسالة في المحادثة قد تُجاب من موديل مختلف عشوائيًا (يفسّر ضعف تتبّع السياق وتكرار الردود).
...
## جلسة 2026-06-18 — "إنشاء مدير المشروع" بيعلّق ولا يرجّع أي رد

المستخدم اختار الموديلات، حفظ، ضغط "🤖 إنشاء مدير المشروع" — الزر علّق بدون أي رد حتى بعد دقائق.

### السبب الجذري
`infrastructure/docker-compose.yml`: متغيّر `OPENWEBUI_URL` الخاص بـ `tools-api` كان:
```
OPENWEBUI_URL=${OPENWEBUI_URL:-http://ai-open-webui:8888}
```
الـ `8888` هو **البورت المنشور للمستخدم على الـ host فقط** (`8888:8080` mapping) — البورت الداخلي الحقيقي جوه شبكة Docker هو `8080`. بما إن `OPENWEBUI_URL` نفسها غير معرّفة كمتغيّر substitution، كانت القيمة الافتراضية الخاطئة (`8888`) تُستخدم دائماً، فحلقة `wait_ready()` في `setup-openwebui.py` كانت تفشل بصمت في كل محاولة (لمدة تصل حتى 300 ثانية) قبل ما ترجع فشل نهائي.

### ⚠️ ملاحظة مهمة عن منهجية العمل
اكتُشفت هذه المشكلة أثناء التشخيص أن **نسخة الريبو المحلية** التي كنت أستخدمها للتعديل في هذه المحادثة كانت **غير متطابقة مع GitHub الحقيقي** لهذا السطر بالتحديد (كانت قديمة). تم عمل `git clone` جديد بالكامل للتأكد من التطابق 100%، وتأكدنا أن كل فيكسات هذه الجلسة (retry logic، run-history، إلخ) كانت قد اتُّرفعت بنجاح فعليًا، والمشكلة محصورة في هذا السطر القديم فقط. **الدرس:** لأي تشخيص حسّاس، التحقق من المحتوى الحقيقي على GitHub مباشرة (API) أو عمل clone جديد، بدل الاعتماد على نسخة محلية قد تكون قديمة.

### الإصلاح
1. `docker-compose.yml`: `OPENWEBUI_URL=http://ai-open-webui:8080` (قيمة ثابتة صحيحة، بدون fallback مُضلِّل)
2. `setup-openwebui.py`: تقليل `wait_ready`'s `max_wait` من 300 إلى 60 ثانية — فشل سريع بدل الانتظار 5 دقائق
3. `tools-api/server.py`: استدعاء `subprocess.run(["python3", script_path])` مباشرة بدل التغليف بـ `bash -c` (أكثر موثوقية لقتل العملية عند انتهاء الـ timeout)

---
---

## جلسة 2026-06-17 (تابع) — تخفيف rate-limit الموديلات المجانية في OpenRouter

موديلات OpenRouter المجانية (":free") تُرفض كثيراً بـ HTTP 429 لأنها مشتركة بين آلاف المستخدمين. هذا قيد حقيقي من المزوّد نفسه — لا يوجد "إصلاح جذري" بالكامل، لكن أُضيف تخفيف عملي:

- `pipeline.py` (الـ 6 agents): عند 429، تُقرأ مدة `Retry-After` من رد الخطأ نفسه، وتُعاد المحاولة (حتى 3 مرات) بعد الانتظار
- `crew.py` (توليد التصاميم): نفس المنطق عند `litellm.RateLimitError`، بانتظار ثابت 9 ثوانٍ

### ملاحظة للمستخدم
لو استخدمت موديل مجاني واحد لكل الـ agents في نفس الوقت، الضغط على نفس الموديل يزيد من احتمال الـ rate-limit. الأفضل توزيع الـ agents على موديلات مجانية مختلفة (كما فعلنا في هذه الجلسة).
